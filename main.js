...
const download = document.querySelectorAll('.fa-circle-down');
...
const songs = [
  ...
  function imageUpdate() {
    ...
    slider.src = `images/${songs[i].thumbnail}`;
  }
  ...
  function nextImage() {
    ...
    for (let k = 0; k < songs.length; k++) {
      ...
      if (imageSource === songs[k].thumbnail) {
        ...
        if (k !== songs.length-1) {
          maxPlayerImage.src = maxPlayerImage.src.replace(imageSource, songs[k+1].thumbnail);
        ...
        //if the match is the last image in the array, display the first image; change titles accordingly
          ...
          maxPlayerImage.src = maxPlayerImage.src.replace(imageSource, songs[0].thumbnail);
    ...
    for(let l=0; l<songs.length; l++) {
      ...
      if(mediaSource === songs[l].audio){
        ...
        //if the match is the first song in the array, play the last song
          ...
          media.src = media.src.replace(mediaSource, songs[songs.length-1].audio);
          pauseBtn.style.display = 'block';
          playBtn.style.display = 'none';
          //provisions for minplayer
          minPlay.style.display = 'none';
  ...
  function previousImage() {
    ...
    for (let m = 0; m < songs.length; m++) {
      ...
      if (imageSource === songs[m].thumbnail) {
        ...
        if (m !== 0) {
          maxPlayerImage.src = maxPlayerImage.src.replace(imageSource, songs[m-1].thumbnail);
        ...
        //if the match is the first image in the array, display the last image; change titles accordingly
          ...
          maxPlayerImage.src = maxPlayerImage.src.replace(imageSource, songs[songs.length-1].thumbnail);
  ...
  media.addEventListener('ended', () => {
    if(random == 1) {
      ...
      let imageIndex = maxPlayerImage.src.indexOf('images');
      let imageSource = maxPlayerImage.src.slice(imageIndex + 7);
      maxPlayerImage.src = maxPlayerImage.src.replace(imageSource, songs[o]. thumbnail);
      //provisions for minplayer
      minPlayerImage.src = maxPlayerImage.src;
  ...
  minClose.addEventListener('click', () => {
    ...
    function slide() {
      ...
      if (minPlayer.style.visibility === 'visible') {
        for (let p = 0; p < document.innerWidth; p += (document.innerWidth) / 1000) {
...
function playMusic(e) {
  ...
  for(let q=0; q<songs.length; q++){
    ...
    switch(text) {
      ...
      let imageIndex = maxPlayerImage.src.indexOf('images');
      let imageSource = maxPlayerImage.src.slice(imageIndex + 7);
      maxPlayerImage.src = maxPlayerImage.src.replace(imageSource, songs[q]. thumbnail);
      //provisions for minplayer
      minPlayerImage.src = maxPlayerImage.src;
